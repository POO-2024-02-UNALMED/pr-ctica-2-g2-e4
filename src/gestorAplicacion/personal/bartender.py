...
from gestorAplicacion.personal import Empleado
...
class Bartender(Empleado):
    ...
    def preparar_bebida_bienvenida(self, cliente):
        ...
        if not bebida_base:
            bebida_base = max(Bartender.barra_de_bebidas, key=lambda b: b.get_favorito(), default=None)
    ...
    def preparar_bebida(self, nombre_bebida: str, suscripcion: Suscripcion):
        bebida_base = next((b for b in Bartender.barra_de_bebidas if b.get_nombre().lower() == nombre_bebida.lower()), None)
        if not bebida_base:
            raise ValueError("La bebida solicitada no está disponible en la barra.")

        ingredientes_preparados = []
        for ingrediente in bebida_base.get_ingredientes():
            encontrado = next((i for i in Bartender.barra_de_ingredientes if i.get_nombre().lower() == ingrediente.get_nombre().lower()), None)
            if encontrado:
                ingredientes_preparados.append(Ingrediente(encontrado.get_nombre(), suscripcion))
            else:
                raise ValueError(f"No se encontraron todos los ingredientes necesarios para la bebida: {ingrediente.get_nombre()}")
    ...
    def generar_menu(self, alcoholico: bool, dulce: bool, amargo: bool, acido: bool, bebida_favorita: Bebida, suscripcion: Suscripcion) -> str:
        self.menu_actual = []
        menu = [f"Menú personalizado (Descuento por suscripción: {suscripcion.get_descuento() * 100}%):"]
        ...
        for bebida in Bartender.barra_de_bebidas:
            if ((not dulce or bebida.is_dulce()) and (not amargo or bebida.is_amargo()) and
                ...
                precio_descuento = int(bebida.get_precio() * (1 - suscripcion.get_descuento()))
